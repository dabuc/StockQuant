import click
from stockquant.odl.baostock import bs_daily, bs_daily_hfq, bs_min_hfq, bs_sz50_stocks, bs_weekly_hfq
...
@cli.command()
@click.option("--reset", type=click.BOOL, default=False, help="是否重置任务列表，默认否")
def update_m60_hfq(reset):
    """更新BS60分钟线后复权数据"""
    click.confirm("正在更新BS60分钟线后复权数据，是否继续？", abort=True)
    if reset:
        bs_min_hfq.update_task()
    bs_min_hfq.get_min_hfq()

    click.echo("更新BS60分钟线后复权数据更新完成。")
